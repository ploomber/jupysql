...
def sample_db(tmp_empty):
    ...
    conn.session.execute(text("CREATE TABLE one (x INT, y TEXT)"))
    conn.session.execute(text("CREATE TABLE another (i INT, j TEXT)"))
    ...
    conn.session.execute(text("ATTACH DATABASE 'my.db' AS schema"))
